<?php
	...
	if(isset($_POST['add'])){
		...
		$tworca = mysqli_real_escape_string($conn, $tworca);
		...
		$query = "INSERT INTO series VALUES ('" . $id . "', '" . $tytul . "', '" . $tworca . "', '" . $image . "', '" . $opis . "', '" . $cena . "', '" . $premiera . "')";
...
?><div class="container">
	...
	<form method="post" action="admin_add_series.php" enctype="multipart/form-data">
		<table class="table">
			<tr>
				<th>ID </th>
			...
			<tr>
				<th>Reżyser</th>
				<td><input type="text" name="tworca" required></td>
			</tr>
			<tr><th>Miniaturka (../img/series)</th>
		...
		<input type="submit" name="add" value="Dodaj film" class="btn btn-success">
		<input type="reset" value="Wyczyść" class="btn btn-danger"><br><br>
		<a href="admin_series.php" class="btn btn-primary"><i class="fas fa-backward"></i> Powrót</a>
	</form>
            <br/></div></div>
<?php
